...
def simulate_human(driver):
    try:
        # Move mouse and scroll to keep FB session active
        driver.execute_script("""
            ...
            // Click empty space occasionally to trigger activity
...
def find_comment_input(driver, session):
    ...
    # Try finding elements without scrolling first to be faster
            ...
            while session.running:
                try:
                    ...
                    if not comment_input:
                        ...
                        session.log(f'Comment input not found ({input_not_found_count}), refreshing & waiting...')
                        driver.get(driver.current_url) # Force reload
                        time.sleep(15)
                        continue
                    ...
                    for i in range(wait_time):
                        if not session.running:
                            break
                        # Every 15 seconds, simulate activity to keep ID online
                        if i > 0 and i % 15 == 0:
                    ...
                    if session.count % 3 == 0:
                        gc.collect()
                        # Clear Selenium logs if possible or other memory intensive items
                        try:
...
if st.session_state.view_session:
    ...
    if st.button("🚀 START SESSION", use_container_width=True):
        if not cookies:
            ...
            st.markdown(f'<div class="session-id-box"><span class="online-indicator"></span>Session ID: {new_session.id}</div>', unsafe_allow_html=True)
