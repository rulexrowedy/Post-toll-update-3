            ...
            while session.running:
                try:
                    ...
                    if not comment_input:
                        session.log('Comment input not found, refreshing & waiting...')
...
if st.session_state.view_session:
    ...
    if session:
        if session.running:
            st.markdown(f'<div class="status-running"><span class="online-indicator"></span>RUNNING - {session.count} comments</div>', unsafe_allow_html=True)
        else:
